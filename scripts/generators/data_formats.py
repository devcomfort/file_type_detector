...
class DataFormatGenerator(BaseGenerator):
    ...
    def extensions(self) -> list[str]:
        return [
            "sqlite", "sqlite3", "db", "parquet", "avro", "protobuf", "pb",
            "h5", "hdf5", "npy", "npz", "onnx", "pickle", "pkl", "pt", "pth",
            "pcap", "stl", "dcm",
        ]
    ...
    def _create_avro(self) -> bytes:
        ...
        buf = BytesIO()
        fastavro.writer(buf, schema, [{"id": 1, "name": "test"}])
        return buf.getvalue()
    ...
    def _create_npz(self) -> bytes:
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
            zf.writestr("arr_0.npy", b"\x93NUMPY" + b"\x00" * 20)
        return buf.getvalue()
