...
class CodeFormatGenerator(BaseGenerator):
    ...
    def extensions(self) -> list[str]:
        return [
            "py", "pyi", "js", "mjs", "cjs", "ts", "mts", "cts", "tsx", "mtsx", "ctsx",
            "jsx", "mjsx", "cjsx", "java", "kt", "kts", "scala", "rb", "php", "sh",
            "lua", "swift", "go", "rs", "c", "cc", "cxx", "cpp", "c++", "cppm", "ixx",
            "h", "hh", "hpp", "hxx", "h++", "cs", "csx", "m", "mm", "f90", "f95", "f03",
            "v", "verilog", "vlg", "vh", "vhd", "sql", "pl", "pro", "p", "pas", "pp",
            "jl", "R", "dart", "ex", "exs", "erl", "hrl", "hs", "lhs", "ml", "mli",
            "lisp", "lsp", "l", "cl", "scm", "ss", "coffee", "sol", "yar", "yara",
            "hcl", "proto", "textproto", "textpb", "pbtxt", "cmake", "gradle", "groovy",
            "bzl", "gemspec", "au3", "awk", "hbs", "handlebars", "jinja", "jinja2", "j2",
            "scss", "odin", "gleam", "ipynb", "tcl", "m4", "bf", "b", "smali", "abnf",
            "aidl", "gpx", "xsd", "htm", "xhtml", "xht", "tsv", "jsonld", "yml",
            "s", "S", "asm", "cob", "cpy", "cbl", "CBL", "COB", "CPY", "F90", "P", "a68",
            "html", "css", "json", "jsonl", "svg", "yaml", "toml", "ini",
            "dockerfile", "makefile", "bat", "ps1", "vbs", "ksh",
            "erb", "vue", "clj", "litcoffee", "elm", "nix", "tf",
            "vim", "aff", "zig", "csproj", "vcxproj", "hta", "jnlp",
            "twig", "vba", "mht", "raku", "raku6", "p6", "pl6", "pm6",
            "cabal", "dhall", "purs", "agda", "idr", "lean", "roc",
            "mojo", "carbon", "move", "cairo", "wgsl", "glsl", "hlsl",
            "metal", "spv", "mlir", "firrtl", "chisel", "bluespec",
            "vb", "vbe", "asp", "aspx", "matlab", "cljr", "cljs",
            "r", "rust", "po", "pot", "csv", "xml",
        ]

    @property
    def sources(self) -> dict[str, str]:
        base = {ext: "synthetic:Minimal valid source code" for ext in [
            "py", "pyi", "js", "mjs", "cjs", "ts", "mts", "cts", "tsx", "mtsx", "ctsx",
            "jsx", "mjsx", "cjsx", "java", "kt", "kts", "scala", "rb", "php", "sh",
            "lua", "swift", "go", "rs", "c", "cc", "cxx", "cpp", "c++", "cppm", "ixx",
            "h", "hh", "hpp", "hxx", "h++", "cs", "csx", "m", "mm", "f90", "f95", "f03",
    ...
    def generate(self, ext: str) -> bytes:
        generators = {
            "py": self._py, "pyi": self._pyi,
            "js": self._js, "mjs": self._js, "cjs": self._js,
            "ts": self._ts, "mts": self._ts, "cts": self._ts,
            "tsx": self._tsx, "mtsx": self._tsx, "ctsx": self._tsx,
            "jsx": self._jsx, "mjsx": self._jsx, "cjsx": self._jsx,
            "java": self._java, "kt": self._kt, "kts": self._kt,
            "scala": self._scala, "rb": self._rb, "php": self._php,
            "sh": self._sh, "lua": self._lua, "swift": self._swift,
            "go": self._go, "rs": self._rs, "c": self._c,
            "cc": self._cpp, "cxx": self._cpp, "cpp": self._cpp,
            "c++": self._cpp, "cppm": self._cpp, "ixx": self._cpp,
            "h": self._h, "hh": self._hpp, "hpp": self._hpp,
            "hxx": self._hpp, "h++": self._hpp,
            "cs": self._cs, "csx": self._cs,
            "m": self._objc, "mm": self._objc,
            "f90": self._fortran, "f95": self._fortran, "f03": self._fortran,
            "v": self._verilog, "verilog": self._verilog, "vlg": self._verilog, "vh": self._verilog,
            "vhd": self._vhdl, "sql": self._sql, "pl": self._perl,
            "pro": self._prolog, "p": self._prolog, "pas": self._pascal, "pp": self._pascal,
            "jl": self._julia, "R": self._r, "dart": self._dart,
            "ex": self._elixir, "exs": self._elixir,
            "erl": self._erlang, "hrl": self._erlang_hrl,
            "hs": self._haskell, "lhs": self._haskell_lhs,
            "ml": self._ocaml, "mli": self._ocaml_mli,
            "lisp": self._lisp, "lsp": self._lisp, "l": self._lisp, "cl": self._lisp,
            "scm": self._scheme, "ss": self._scheme,
            "coffee": self._coffee, "sol": self._solidity,
            "yar": self._yara, "yara": self._yara,
            "hcl": self._hcl, "proto": self._proto,
            "textproto": self._textproto, "textpb": self._textproto, "pbtxt": self._textproto,
            "cmake": self._cmake, "gradle": self._gradle, "groovy": self._groovy,
            "bzl": self._bzl, "gemspec": self._gemspec,
            "au3": self._autoit, "awk": self._awk,
            "hbs": self._handlebars, "handlebars": self._handlebars,
            "jinja": self._jinja, "jinja2": self._jinja, "j2": self._jinja,
            "scss": self._scss, "odin": self._odin, "gleam": self._gleam,
            "ipynb": self._ipynb, "tcl": self._tcl, "m4": self._m4,
            "bf": self._brainfuck, "b": self._brainfuck,
            "smali": self._smali, "abnf": self._abnf,
            "aidl": self._aidl, "gpx": self._gpx, "xsd": self._xsd,
            "htm": self._html, "xhtml": self._html, "xht": self._html,
            "tsv": self._tsv, "jsonld": self._jsonld, "yml": self._yaml,
            "s": self._asm, "S": self._asm, "asm": self._asm,
            "cob": self._cobol, "cpy": self._cobol, "cbl": self._cobol,
            "html": self._html, "css": self._css, "json": self._json,
            "jsonl": self._jsonl, "svg": self._svg, "yaml": self._yaml,
            "toml": self._toml, "ini": self._ini,
            "dockerfile": self._dockerfile, "makefile": self._makefile,
            "bat": self._bat, "ps1": self._ps1, "vbs": self._vbs, "ksh": self._ksh,
            "erb": self._erb, "vue": self._vue, "clj": self._clj,
            "litcoffee": self._litcoffee, "elm": self._elm, "nix": self._nix,
            "tf": self._tf, "vim": self._vim, "aff": self._aff,
            "zig": self._zig, "csproj": self._csproj, "vcxproj": self._vcxproj,
            "hta": self._hta, "jnlp": self._jnlp, "twig": self._twig,
            "vba": self._vba, "mht": self._mht,
            "raku": self._raku, "raku6": self._raku, "p6": self._raku,
            "pl6": self._raku, "pm6": self._raku,
            "cabal": self._cabal, "dhall": self._dhall, "purs": self._purs,
            "agda": self._agda, "idr": self._idr, "lean": self._lean,
            "roc": self._roc, "mojo": self._mojo, "carbon": self._carbon,
            "move": self._move, "cairo": self._cairo, "wgsl": self._wgsl,
            "glsl": self._glsl, "hlsl": self._hlsl, "metal": self._metal,
            "spv": self._spv, "mlir": self._mlir, "firrtl": self._firrtl,
            "chisel": self._chisel, "bluespec": self._bluespec,
            "vb": self._vb, "vbe": self._vbe, "asp": self._asp, "aspx": self._aspx,
            "matlab": self._matlab, "cljr": self._cljr, "cljs": self._cljs,
            "CBL": self._cobol_upper, "COB": self._cobol_upper, "CPY": self._cobol_copy_upper,
            "F90": self._fortran, "P": self._prolog_upper, "a68": self._algol68,
            "r": self._r_lower, "rust": self._rs, "po": self._po,
            "pot": self._pot, "csv": self._csv, "xml": self._xml,
        }
