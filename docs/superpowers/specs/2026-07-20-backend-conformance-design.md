...
An extension alone does not identify a file format reliably. Each inventory record binds one checksum-verified content fixture to one declared probe extension. Alias coverage uses a separate record for each probe extension; records may share the same fixture bytes and checksum.

`tests/truth/backend_inventory.json` is the authoritative source for collection. Each entry follows this shape:
...
```json
{
  ...
  "provenance": "Generated by py7zr in scripts/generators/archives.py:ArchiveGenerator._create_7z",
  "ground_truth_review": {
    "status": "verified",
    "reviewed_by": "reviewer",
    "reviewed_at": "2026-07-21",
    "evidence": ["https://www.7-zip.org/7z.html"]
  },
  "backends": ["lexical", "magic", "magika", "hybrid"]
}
```

The loader verifies root-relative fixture paths, SHA-256 values, the fixed backend list, and the review record. A verified candidate requires one fact-identical authoritative record, and every authoritative record requires that verified candidate. Authoritative records require lowercase MIME types and extensions plus reviewer, date, and evidence. A suffix counts as covered only when an authoritative record supplies all fields. The report shows both record coverage and unique-suffix coverage.

The current canonical fixture data is candidate migration input, not a competing source of truth. A checksum proves fixture stability, not that the declared MIME type or extension is correct. `needs_review` and `excluded` candidates preserve raw legacy claims, including noncanonical casing, with a stated reason. They remain outside collection until independent review supplies canonical Ground Truth. Detector output must never create or correct Ground Truth because that would make the test circular.

`backend_inventory.json` contains only `ground_truth_review.status: verified` records. Conflicting or ambiguous entries remain in `tests/truth/backend_inventory_candidates.json` with `needs_review` status and no conformance observation. The current candidates explicitly retain the `.txt` claim for `sample.aiff`, the `.mp4` claim for `sample.avif`, and the corrected `py7zr` provenance for `sample.7z`. The current tool snapshot is historical drift data and will not decide correctness after the inventory cutover.
